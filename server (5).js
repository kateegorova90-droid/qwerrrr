...
// ── WB HTTPS запрос ───────────────────────────────────────────────────────────
function wbReq(hostname, path, method, body) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end",   () => {
        ...
        catch { resolve({ _raw: raw.slice(0, 500), _status: res.statusCode }); }
      });
      res.on("error", reject);
    });
...
const ROUTES = {
  ...
  "POST /api/adv/fullstats": {
    ttl: 300,
    fn:  (q, body) => wbReq("advert-api.wb.ru", "/adv/v2/fullstats", "POST", body),
  },

  // Баланс рекламного кабинета
  "GET /api/adv/balance": {
    ttl: 60,
    fn:  () => wbReq("advert-api.wb.ru", "/adv/v1/balance"),
  },
