    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end",   () => {
        ...
        if (res.statusCode === 429 && attempt < 4) {
          ...
          console.log(`[WB] 429 на ${hostname}${path}, retry ${attempt} через ${delay}ms`);
          setTimeout(() => {
        ...
        if (res.statusCode >= 400) {
          console.error(`[WB] HTTP ${res.statusCode} на ${hostname}${path}:`, raw.slice(0, 200));
        }
...
const ROUTES = {
  ...
  "POST /api/adv/fullstats": {
    ttl: 300,
    fn:  (q, body) => wbReq("advert-api.wildberries.ru", "/adv/v2/fullstats", "POST", body),
  },

  // Баланс рекламного кабинета
  "GET /api/adv/balance": {
    ttl: 60,
    fn:  () => wbReq("advert-api.wildberries.ru", "/adv/v1/balance"),
  },
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (_inFlight > 0 && pathname !== "/health") {
      await new Promise(r => setTimeout(r, 300 * _inFlight));
    }
    _inFlight++;
    
    let data;
    try {
      data = await route.fn(query, body);
    } finally {
