    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end",   () => {
        ...
        if (res.statusCode === 429 && attempt <= 5) {
          const retryAfter = parseInt(res.headers["x-ratelimit-retry"] || res.headers["retry-after"] || "10", 10);
          const wait = retryAfter * 1000;
          console.log(`[WB] 429 ${hostname}${path} — ждём ${retryAfter}s (попытка ${attempt})`);
          setTimeout(() => {
