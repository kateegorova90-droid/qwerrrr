  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end",   () => {
        ...
        catch { resolve({ status: res.statusCode, data: { error: raw.slice(0, 500) } }); }
      });
      res.on("error", reject);
    });
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && pathname === "/api/claude") {
    if (!ANTHROPIC_KEY) {
      ...
      return res.end(JSON.stringify({ error: "ANTHROPIC_API_KEY не задан в Railway Variables" }));
    }
    try {
      const body = await readBody(req);
      const result = await claudeProxy(body);
